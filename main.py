...
from models.cvr_ccp import ONLY_CVR as OCVR
#For reproducibility
...
def get_model(name, categorical_field_dims, numerical_num, task_num, expert_num, embed_dim):
    ...
    """
    if name == 'only_cvr' and 'ccp' in args.dataset_name:
        print("Model: DDPO")
        print("Dataset:",args.dataset_name)
        return OCVR(categorical_field_dims, numerical_num, embed_dim=embed_dim, bottom_mlp_dims=(128, 64), tower_mlp_dims=(64, 32), task_num=1, dropout=0.2)
    elif name == 'ddpo' and 'ccp' in args.dataset_name:
        print("Model: DDPO")
        print("Dataset:",args.dataset_name)
        return DDPO_C(categorical_field_dims, numerical_num, embed_dim=embed_dim, bottom_mlp_dims=(256, 128), tower_mlp_dims=(128, 64), task_num=task_num, dropout=0.2)
...
def train(model, optimizer, data_loader, criterion, device,parao,parac,parar,paract,log_interval=100):
    ...
    for i, (categorical_fields, numerical_fields, labels) in enumerate(loader):
        ...
        y = model(categorical_fields.to(torch.long), numerical_fields)
        
        ctr_loss = criterion(y[0], labels[:, 0].float())
        ctuncvr_label =  torch.where((labels[:,0] == 1) & (labels[:, 1] == 0), torch.ones_like(labels[:,0]), torch.zeros_like(labels[:,0]))
        ctuncvr_loss = criterion(torch.mul(y[0],y[2]), ctuncvr_label.float())
        ctcvr_loss = criterion(torch.mul(y[0],y[1]), labels[:, 2].float())
        ctr_loss = torch.mean(ctr_loss)
        ctcvr_loss = torch.mean(ctcvr_loss)
        ctuncvr_loss = torch.mean(ctuncvr_loss)

        uncvr = y[2].detach()
        labels_cvr = labels[:, 1].float()
        labels_cvr_soft = 1-uncvr
        labels_cvr_soft.detach()
        cvr_loss_soft = criterion(y[1], labels_cvr_soft)
        cvr_loss = criterion(y[1], labels_cvr)
        O = labels[:, 0].float()
        cvr_loss = torch.mul(O,cvr_loss)
        cvr_loss_soft = torch.mul(O,cvr_loss_soft)

        cvr = y[1].detach() 
        labels_uncvr = labels[:, 1].float()
        labels_uncvr = (1.0-labels[:, 1]).float()
        labels_uncvr_soft = 1-cvr
        labels_uncvr_soft.detach()
        uncvr_loss_soft = criterion(y[2], labels_uncvr_soft)
        uncvr_loss = criterion(y[2], labels_uncvr)
        #O = labels[:, 0].float()
        uncvr_loss = torch.mul(O,uncvr_loss)
        uncvr_loss_soft = torch.mul(O,uncvr_loss_soft)
        ...
        cvr_loss_soft = torch.mul(IPS,cvr_loss_soft)
        cvr_loss = torch.mean(cvr_loss)
        cvr_loss_soft = torch.mean(cvr_loss_soft)

        uncvr_loss = torch.mul(IPS,uncvr_loss)
        uncvr_loss_soft = torch.mul(IPS,uncvr_loss_soft) 
        uncvr_loss = torch.mean(uncvr_loss)
        uncvr_loss_soft  = torch.mean(uncvr_loss_soft)
   
        side_cvr_loss = criterion(y[1], 1.0-y[2].detach())
        side_cvr_loss = torch.mul(1-O,side_cvr_loss)

        side_uncvr_loss = criterion(y[2], 1.0-y[1].detach())
        side_uncvr_loss = torch.mul(1-O,side_uncvr_loss)
        ...
        side_uncvr_loss = torch.mul(side_IPS,side_uncvr_loss)
        side_uncvr_loss = torch.mean(side_uncvr_loss)


        loss = parao*ctr_loss + paract*(ctcvr_loss+ctuncvr_loss) + parar*(cvr_loss + cvr_loss_soft + side_cvr_loss) +parac*(uncvr_loss +uncvr_loss_soft+ side_uncvr_loss)
...
def main(dataset_name,
    ...
    early_stopper = EarlyStopper(num_trials=2, load_path=save_path)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_name', default='ddpo', choices=['ddpo', 'only_cvr'])
