...
from datasets.dataset import AliCCPDataset
from models.ccvr_ccp import CCVR as CCVR
...
def get_model(name, categorical_field_dims, numerical_num, task_num, expert_num, embed_dim):
    ...
    """
    if name == 'ccvr' and 'ccp' in args.dataset_name:
        print("Dataset:",args.dataset_name)
        return CCVR(categorical_field_dims, numerical_num, embed_dim=embed_dim, bottom_mlp_dims=(128, 64), tower_mlp_dims=(64, 32), task_num=1, dropout=0.2)
    else:
...
def train(model, optimizer, data_loader, criterion, device,parao,parac,parar,paract,log_interval=100):
    ...
    for i, (categorical_fields, numerical_fields, labels) in enumerate(loader):
        ...
        uncvr = y[2].detach()
        labels_cvr = labels[:, 1].float()+(1.0-uncvr)
        ...
        cvr = y[1].detach()
        labels_uncvr = (1.0-labels[:, 1]).float()+(1.0-cvr)
        labels_uncvr = torch.minimum(labels_uncvr,torch.full_like(labels_uncvr,1))
        labels_uncvr.detach()
        ...
        loss = parao*ctr_loss + paract*(ctcvr_loss+ctuncvr_loss) + parar*(cvr_loss + side_cvr_loss) +parac*(uncvr_loss + side_uncvr_loss)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset_name', default='ccp', choices=['ccp','industrial'])
    parser.add_argument('--dataset_path', default='./data/')
    parser.add_argument('--model_name', default='ccvr', choices=['ccvr', 'ccvr_v1', 'ccvr_v2'])
