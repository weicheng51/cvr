...
class AliExpressDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, dataset_path):
        ...
        self.labels = np.concatenate([self.labels,data[:, -1:]],axis=1)
...
class AliCCPDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, dataset_path):
        ...
        self.categorical_data = data[:, 2:].astype(np.int32)
        self.labels = data[:,:2].astype(np.float32)
        self.labels = np.concatenate([self.labels,data[:, 1:2]],axis=1)
    ...
    def __getitem__(self, index):
        return self.categorical_data[index], self.categorical_data[index], self.labels[index]
